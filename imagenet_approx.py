...
def imagenet_maml_exp():
    ...
    for iter in range(num_iterations):
        ...
        # 2. for each task Ti
        meta_learning_loss = 0 # for printout not for training
        meta_grads = [None for _ in range(metabatch_size)]
        for ti, task in enumerate(tasks):
            ...
            meta_grad = torch.autograd.grad(meta_loss, fast_weights, create_graph=True)
            meta_grads[ti] = meta_grad
        ...
        for i, param in enumerate(imagenet_learner.parameters()):
            gr = meta_grads[0][i].data
            for j in range(1, metabatch_size):
                gr += meta_grads[j][i].data

            param.grad = gr/num_tasks # should have divided this by meta_batchsize OR no division at all
                                      # but still do this to be consistent with the no-approx result

        optimizer.step()
        optimizer.zero_grad()

        if iter % 5000 == 0:
            savepath = "trained_models/imagenet_approx_5march_n{}_k{}_iter{}.pt".format(num_tasks, num_points, iter)
            print("saving a model at", savepath)
            torch.save(imagenet_learner.state_dict(), savepath)

    savepath = "trained_models/imagenet_approx_5march_n{}_k{}_final.pt".format(num_tasks, num_points)
    print("saving a model at", savepath)
    torch.save(imagenet_learner.state_dict(), savepath)
