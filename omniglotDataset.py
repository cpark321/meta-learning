...
class OmniglotDataset(data.Dataset):
    ...
    def getOmniglotArray(self):
        X_data = np.zeros((1623, 20, 28, 28), dtype=np.uint8)
        idx=0

        datapath = './processed/'
        datasplits = os.listdir(datapath)

        for datasplit in datasplits:
            data_dir = os.path.join(datapath, datasplit)
            alphbets = os.listdir(data_dir)
            for alphabet in alphbets:
                ap_dir = os.path.join(data_dir, alphabet)
                characters = os.listdir(ap_dir)

                for character in characters:
                    char_dir = os.path.join(ap_dir, character)
